...
;(function (factory) {
  ...
  $.jstree.core.prototype = {
                      ...
                      if (this._data.core.selected.length) {
                        if (this.settings.core.expand_selected_onload) {
                          ...
                          for (i = 0, j = this._data.core.selected.length; i < j; i++) {
                            tmp = tmp.concat(
                              this._model.data[this._data.core.selected[i]].parents
                            )
                          }
    ...
    open_all: function (obj, animation, original_obj) {
      ...
      var dom =
          obj.id === $.jstree.root ? this.get_container_ul() : this.get_node(obj, true),
        i,
        j,
        _this
      if (!dom.length) {
    ...
    copy_node: function (obj, par, pos, callback, is_loaded, skip_redraw, origin) {
      ...
      if ($.vakata.is_array(obj)) {
        if (obj.length === 1) {
          ...
          for (t1 = 0, t2 = obj.length; t1 < t2; t1++) {
            if (
              (tmp = this.copy_node(obj[t1], par, pos, callback, is_loaded, true, origin))
            ) {
              par = tmp
              pos = 'after'
            }
    ...
    set_theme: function (theme_name, theme_url) {
      ...
      if (theme_url && $.inArray(theme_url, themes_loaded) === -1) {
        $('head').append(
          '<' + 'link rel="stylesheet" href="' + theme_url + '" type="text/css" />'
        )
    ...
    set_theme_variant: function (variant_name) {
      if (this._data.core.themes.variant) {
        this.element.removeClass(
          'jstree-' + this._data.core.themes.name + '-' + this._data.core.themes.variant
        )
      }
    ...
    set_icon: function (obj, icon) {
      ...
      old = obj.icon
      obj.icon =
        icon === true || icon === null || icon === undefined || icon === '' ? true : icon
      dom = this.get_node(obj, true)
        .children('.jstree-anchor')
    ...
    show_icon: function (obj) {
      ...
      dom = this.get_node(obj, true)
      obj.icon = dom.length
        ? dom.children('.jstree-anchor').children('.jstree-themeicon').attr('rel')
        : true
      if (!obj.icon) {
        obj.icon = true
      }
  ...
  $.jstree.plugins.checkbox = function (options, parent) {
                  ...
                  for (i = 0, j = dpc.length; i < j; i++) {
                    if (m[dpc[i]].state[t ? 'selected' : 'checked']) {
                      for (k = 0, l = m[dpc[i]].children_d.length; k < l; k++) {
                        m[m[dpc[i]].children_d[k]].state[t ? 'selected' : 'checked'] =
                          true
                      }
    ...
    this.delete_node = function (obj) {
      ...
      if (c) {
        this._data.checkbox.selected = $.vakata.array_filter(
          this._data.checkbox.selected,
          function (v) {
            return $.inArray(v, tmp) === -1
          }
        ...
        $.each(o, function (i, val) {
          ...
          if (!sep && val.separator_before) {
            str +=
              ...
              "a href='#' " +
              ($.vakata.context.settings.icons ? '' : 'class="vakata-context-no-icons"') +
              '>&#160;<' +
              '/a><' +
              '/li>'
          }
          ...
          if ($.vakata.context.settings.icons) {
            ...
            if (val.icon) {
              if (val.icon.indexOf('/') !== -1 || val.icon.indexOf('.') !== -1) {
                str +=
                  ' style=\'background:url("' + val.icon + '") center center no-repeat\' '
              } else {
                str += " class='" + val.icon + "' "
              }
            }
    ...
    $(function () {
      ...
      vakata_context.element
        ...
        .on('click', 'a', function (e) {
          ...
          if (
            !$(this).trigger('blur').parent().hasClass('vakata-context-disabled') &&
  ...
  $.jstree.plugins.dnd = function (options, parent) {
    this.init = function (el, options) {
      ...
      this.settings.dnd.use_html5 =
        this.settings.dnd.use_html5 && 'draggable' in document.createElement('span')
    }
    this.bind = function () {
      parent.bind.call(this)
        ...
        if (ins && ins._data && ins._data.dnd) {
          ...
          if (
            ...
            (ins.get_container_ul().children().length === 0 ||
              ins.settings.dnd.blank_space_drop)
          ) {
            ok = true
              ...
              $.each(o, function (j, v) {
                ...
                if (ok) {
                  ...
                  lastmv = {
                    ...
                    par: p,
                    pos:
                      v === 'i' && ip === 'last' && i === 0 && !ins.is_loaded(tm)
                        ? 'last'
                        : i,
                  }
  ...
  $.jstree.plugins.types = function (options, parent) {
    ...
    this.get_json = function (obj, options, flat) {
      ...
      if ($.vakata.is_array(tmp)) {
        for (i = 0, j = tmp.length; i < j; i++) {
          tmp[i].type =
            (tmp[i].id || tmp[i].id === 0) && m[tmp[i].id] && m[tmp[i].id].type
              ? m[tmp[i].id].type
              : 'default'
          if (options && options.no_id) {
            delete tmp[i].id
        ...
        tmp.type =
          (tmp.id || tmp.id === 0) && m[tmp.id] && m[tmp.id].type
            ? m[tmp.id].type
            : 'default'
        if (options && options.no_id) {
        ...
        for (k in t[old_type].li_attr) {
          if (t[old_type].li_attr.hasOwnProperty(k)) {
            if (k === 'id') {
              ...
              m[obj.id].li_attr['class'] = (m[obj.id].li_attr['class'] || '').replace(
                t[old_type].li_attr[k],
                ''
              )
              if (d) {
        ...
        for (k in t[old_type].a_attr) {
          if (t[old_type].a_attr.hasOwnProperty(k)) {
            if (k === 'id') {
              ...
              m[obj.id].a_attr['class'] = (m[obj.id].a_attr['class'] || '').replace(
                t[old_type].a_attr[k],
                ''
              )
              if (a) {
  ...
  $.jstree.plugins.unique = function (options, parent) {
    this.check = function (chk, obj, par, pos, more) {
      ...
      switch (chk) {
        ...
        case 'move_node':
          i =
            (obj.parent === par.id && (!more || !more.is_multi)) || $.inArray(n, c) === -1
          if (!i) {
            this._data.core.last_error = {
              error: 'check',
              plugin: 'unique',
